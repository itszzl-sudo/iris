...
## 更新摘要
**所做更改**
- 新增边框渲染支持（DrawCommand::Border变体），实现四边独立边框宽度和颜色控制
- 增强文本渲染基础功能，支持文本节点占位符渲染和字体信息解析
- 扩展批渲染器功能，支持边框和文本渲染命令
- 完善VNode渲染器，新增边框解析和文本渲染逻辑
- 更新着色器系统，支持UV坐标和纹理采样
...
**最新更新**：引擎现已支持边框渲染和文本渲染基础功能，进一步增强了2D图形渲染能力。
...
+drawBorder(border) void
+parseBorderWidth(width) void
+parseBorderColor(color) void
}
class TextRenderer {
+renderText(text) void
+parseTextInfo(info) void
+calculateTextDimensions(text) void
}
class TextureAtlas {
...
**章节来源**
- [batch_renderer.rs:1-435](file://crates/iris-gpu/src/batch_renderer.rs#L1-L435)
...
**章节来源**
- [batch_renderer.rs:209-435](file://crates/iris-gpu/src/batch_renderer.rs#L209-L435)
...
6. **新增边框渲染支持**：实现四边独立的边框宽度和颜色控制
7. **文本渲染基础功能**：支持文本节点占位符渲染和字体信息解析
