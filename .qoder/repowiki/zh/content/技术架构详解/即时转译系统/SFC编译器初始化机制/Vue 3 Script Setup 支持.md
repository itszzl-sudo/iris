...
| 宏名称 | 功能 | 示例 |
|--------|------|------|
| `defineProps<T>()` | 定义组件 props | `defineProps<{ title: string }>()` |
| `defineEmits<T>()` | 定义组件 emits | `defineEmits<{ change: [number] }>()` |
| `defineExpose()` | 暴露组件属性 | `defineExpose({ publicMethod })` |
| `withDefaults()` | 设置 props 默认值 | `withDefaults(defineProps<...>(), { count: 0 })` |
...
Parser->>Converter : 生成 props 定义
Input->>Parser : 解析 defineEmits
Parser->>Converter : 生成 emits 定义
Input->>Parser : 解析顶层声明
