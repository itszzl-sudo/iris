...
**更新** 布局引擎iris-layout已成功移除对iris-gpu的依赖，实现了与渲染器的完全解耦。新增的DOM树操作和虚拟DOM支持模块进一步增强了系统的功能完整性。现在布局引擎可以独立于渲染器运行，支持服务器端布局预计算和离线环境下的布局处理。
...
### 4. DOM树管理
- **DOM节点操作**: 支持插入、删除、替换等操作
- **DOM树索引**: 提供快速节点定位和查询
- **路径管理**: 支持复杂的树形结构导航

### 5. 虚拟DOM系统
- **VNode数据结构**: 轻量级虚拟DOM表示
- **差异比较**: 高效的DOM变更检测
- **补丁应用**: 最小化DOM更新操作
...
FlexLayout["Flex布局引擎"]
FlowLayout["流式布局引擎"]
DOMTree["DOM树管理<br/>节点操作 + 路径索引"]
VNodeSystem["虚拟DOM系统<br/>VNode + Diff + Patch"]
EventSystem["事件系统<br/>事件传播 + 监听器"]
...
**更新** 新增的DOM树管理系统提供了完整的DOM节点操作和管理功能。

#### 核心功能
- **节点操作**: 支持插入、删除、替换等DOM操作
- **路径索引**: 提供快速节点定位和查询
- **父子关系管理**: 维护完整的树形结构关系
...
```mermaid
flowchart TD
CreateNode([创建DOM节点]) --> SetAttributes["设置属性"]
SetAttributes --> AppendChildren["添加子节点"]
AppendChildren --> BuildIndex["构建索引"]
BuildIndex --> QueryNode["查询节点"]
QueryNode --> ModifyNode["修改节点"]
ModifyNode --> UpdateIndex["更新索引"]
UpdateIndex --> End([操作完成])
```

**图表来源**
- [dom.rs:36-238](file://crates/iris-layout/src/dom.rs#L36-L238)
- [domtree.rs:40-317](file://crates/iris-layout/src/domtree.rs#L40-L317)

### 虚拟DOM系统

**更新** 新增的虚拟DOM系统提供了高效的UI更新机制。
...
```mermaid
flowchart TD
OldVNode([旧VNode]) --> CompareTypes["比较节点类型"]
CompareTypes --> TypeMatch{"类型匹配?"}
TypeMatch --> |是| CompareContent["比较内容"]
TypeMatch --> |否| ReplaceNode["替换节点"]
CompareContent --> CompareAttrs["比较属性"]
CompareAttrs --> CompareChildren["比较子节点"]
CompareChildren --> GeneratePatches["生成补丁"]
GeneratePatches --> ApplyPatches["应用补丁"]
ReplaceNode --> GeneratePatches
ApplyPatches --> End([更新完成])
```

**图表来源**
- [vdom.rs:233-358](file://crates/iris-layout/src/vdom.rs#L233-L358)

### 事件系统

**更新** 新增的事件系统提供了完整的DOM事件处理能力。
...
VNodeSystem["虚拟DOM系统<br/>iris-dom"]
EventSystem["事件系统"]
end
subgraph "外部依赖"
...
LayoutStyle --> Core
LayoutStyle --> VNodeSystem
LayoutStyle --> EventSystem
WebGPU --> wgpu
JSRuntime --> quickjs
VNodeSystem --> LayoutStyle
EventSystem --> LayoutStyle
```
...
- **iris-layout** 现在只依赖 `iris-core`、`html5ever`、`cssparser` 和 `iris-dom`
...
4. **事件系统接口**: `Event`和`EventRegistry`处理用户交互
...
### 5. 新增功能的性能优化
- **虚拟DOM差异比较**: 使用高效的Diff算法
- **DOM树索引**: O(log n)的节点查找性能
- **事件监听器缓存**: 减少事件处理开销
...
#### 5. 新增功能相关问题
- **虚拟DOM更新失败**: 检查VNode差异比较逻辑
- **DOM树操作异常**: 验证节点索引和路径管理
- **事件处理问题**: 确认事件监听器注册和传播
...
**更新** 新增功能的集成情况：
- **虚拟DOM系统**: 已完全集成到布局引擎中
- **DOM树管理**: 提供完整的节点操作和查询功能
- **事件系统**: 支持完整的事件处理机制
- **跨模块协作**: 与iris-dom形成完整的UI框架

这些新增功能进一步完善了Leivue Runtime的生态系统，为开发者提供了从底层布局计算到上层UI交互的完整解决方案。