# 文件监控与热重载系统

<cite>
**本文档引用的文件**
- [crates/iris-gpu/src/file_watcher.rs](file://crates/iris-gpu/src/file_watcher.rs)
- [crates/iris-gpu/tests/file_watcher_integration.rs](file://crates/iris-gpu/tests/file_watcher_integration.rs)
- [crates/iris-gpu/src/batch_renderer.rs](file://crates/iris-gpu/src/batch_renderer.rs)
- [crates/iris-gpu/src/batch_shader.wgsl](file://crates/iris-gpu/src/batch_shader.wgsl)
- [crates/iris-sfc/src/lib.rs](file://crates/iris-sfc/src/lib.rs)
- [crates/iris-sfc/src/template_compiler.rs](file://crates/iris-sfc/src/template_compiler.rs)
- [crates/iris-app/src/main.rs](file://crates/iris-app/src/main.rs)
- [crates/iris-core/src/lib.rs](file://crates/iris-core/src/lib.rs)
- [crates/iris-jetcrab-cli/src/server/hmr.rs](file://crates/iris-jetcrab-cli/src/server/hmr.rs)
- [crates/iris-jetcrab-engine/src/hmr.rs](file://crates/iris-jetcrab-engine/src/hmr.rs)
- [crates/iris-jetcrab-cli/src/server/http_server.rs](file://crates/iris-jetcrab-cli/src/server/http_server.rs)
- [crates/iris-jetcrab-cli/src/server/routes.rs](file://crates/iris-jetcrab-cli/src/server/routes.rs)
- [Cargo.toml](file://Cargo.toml)
- [test_hot_reload.md](file://test_hot_reload.md)
- [examples/vue-demo/iris.config.json](file://examples/vue-demo/iris.config.json)
</cite>

## 更新摘要
**变更内容**
- 从旧的项目级热重载系统升级为支持CSS/SCSS样式更新和Vue/TypeScript模块级热替换的新系统
- 新增模块级HMR实现，支持样式热替换和Vue/TS模块热替换
- 更新HMR事件类型和WebSocket通信协议
- 增强文件监控和事件分类处理机制

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构概览](#架构概览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考虑](#性能考虑)
8. [故障排除指南](#故障排除指南)
9. [结论](#结论)

## 简介

Iris 文件监控与热重载系统是一个完整的实时文件变更检测和响应框架，专为 Vue Single File Component (SFC) 开发而设计。该系统提供了毫秒级的热重载体验，支持跨平台文件监控、智能防抖机制、事件去重和优雅的错误处理。

**更新** 系统已升级为模块级热重载架构，支持：
- **CSS/SCSS 样式热替换**：无需重新加载页面即可更新样式
- **Vue 组件模块热替换**：精确替换单个Vue组件，保持应用状态
- **TypeScript/JavaScript 模块热替换**：动态更新脚本模块
- **智能事件分类**：根据文件类型选择合适的HMR策略

系统的核心特性包括：
- **实时文件监控**：使用 notify crate 监听文件系统变化
- **智能防抖**：避免编辑器保存时的重复触发（默认 300ms）
- **事件去重**：同一文件的多次变更只保留最后一次
- **跨平台支持**：Windows、macOS、Linux 完全兼容
- **扩展名过滤**：可配置监听特定文件类型
- **模块级HMR**：支持样式和模块的精确热替换
- **WebSocket通信**：实时推送HMR事件到浏览器

## 项目结构

Iris 采用多 crate 的模块化架构，文件监控与热重载系统主要分布在以下核心 crate 中：

```mermaid
graph TB
subgraph "Iris 核心架构"
Core[iris-core<br/>基础内核]
GPU[iris-gpu<br/>GPU渲染与监控]
SFC[iris-sfc<br/>SFC编译器]
JetCrabCLI[iris-jetcrab-cli<br/>开发服务器]
JetCrabEngine[iris-jetcrab-engine<br/>HMR引擎]
App[iris-app<br/>应用程序入口]
end
subgraph "外部依赖"
Notify[notify crate<br/>文件监控]
Tokio[tokio crate<br/>异步运行时]
Axum[axum crate<br/>HTTP服务器]
WS[websocket<br/>实时通信]
Regex[regex crate<br/>正则表达式]
Broadcast[broadcast channel<br/>事件广播]
end
Core --> GPU
Core --> SFC
Core --> JetCrabCLI
Core --> JetCrabEngine
App --> GPU
App --> SFC
JetCrabCLI --> JetCrabEngine
JetCrabCLI --> Notify
JetCrabCLI --> Tokio
JetCrabCLI --> Axum
JetCrabCLI --> WS
JetCrabEngine --> Broadcast
SFC --> Regex
```

**图表来源**
- [Cargo.toml:1-29](file://Cargo.toml#L1-L29)
- [crates/iris-core/src/lib.rs:1-165](file://crates/iris-core/src/lib.rs#L1-L165)

**章节来源**
- [Cargo.toml:1-29](file://Cargo.toml#L1-L29)

## 核心组件

### 文件监控器 (FileWatcher)

文件监控器是整个热重载系统的核心组件，负责监听文件系统变化并提供异步事件流。

```mermaid
classDiagram
class FileWatcher {
-RecommendedWatcher _watcher
-Receiver~FileChange~ receiver
-WatcherConfig config
-Arc~AtomicBool~ channel_full_warned
-DebounceState debounce
+new(config) Result~FileWatcher~
+recv() async Option~FileChange~
+try_recv() Option~FileChange~
+config() &WatcherConfig
+pending_events() usize
+reset_warning_state() void
}
class WatcherConfig {
+PathBuf watch_path
+bool recursive
+Option~HashSet~ extensions
+usize channel_capacity
+Duration debounce_delay
+new(watch_path) WatcherConfig
+recursive(bool) WatcherConfig
+extensions(Vec~String~) WatcherConfig
+channel_capacity(usize) WatcherConfig
+debounce_delay(Duration) WatcherConfig
}
class FileChange {
<<enumeration>>
Created : PathBuf path
Modified : PathBuf path
Removed : PathBuf path
Renamed : PathBuf from, PathBuf to
+path() &PathBuf
+extension() Option~&str~
}
FileWatcher --> WatcherConfig : uses
FileWatcher --> FileChange : emits
```

**图表来源**
- [crates/iris-gpu/src/file_watcher.rs:172-481](file://crates/iris-gpu/src/file_watcher.rs#L172-L481)

### HMR 管理器 (HMRManager)

**更新** 新增模块级热重载管理器，负责文件监听和HMR事件处理。

```mermaid
classDiagram
class HMRManager {
-ProjectRoot project_root
-Option~RecommendedWatcher~ watcher
-Arc~WebSocketManager~ ws_manager
-bool enabled
+new(project_root, enabled) HMRManager
+ws_manager() Arc~WebSocketManager~
+start_watching(cache) Result~()~
+stop() void
}
class WebSocketManager {
-BroadcastSender~HmrEvent~ tx
+new() WebSocketManager
+subscribe() BroadcastReceiver~HmrEvent~
+broadcast(event) void
}
class HmrEvent {
<<enumeration>>
Connected : message
FileChanged : path, file_name, timestamp
RebuildComplete : cleared_modules, duration_ms
ModuleUpdate : path, module_type, timestamp
StyleUpdate : path, css, timestamp
CompileError : message
NpmDownload : package, version, progress, status, error
}
HMRManager --> WebSocketManager : uses
HMRManager --> HmrEvent : broadcasts
WebSocketManager --> HmrEvent : sends
```

**图表来源**
- [crates/iris-jetcrab-cli/src/server/hmr.rs:117-390](file://crates/iris-jetcrab-cli/src/server/hmr.rs#L117-L390)

### SFC 编译器

SFC 编译器负责将 Vue 单文件组件编译为可执行的 JavaScript 代码。

```mermaid
classDiagram
class SfcModule {
+String name
+String render_fn
+String script
+Vec~StyleBlock~ styles
+u64 source_hash
}
class StyleBlock {
+String css
+bool scoped
+String lang
}
class SfcError {
<<enumeration>>
IoError : source, file
ParseError : message, file, line, column
TemplateError : message, file, line, column
ScriptError : message, file, line, column
}
class SfcCompiler {
+compile(path) Result~SfcModule, SfcError~
+compile_from_string(name, source) Result~SfcModule, SfcError~
-parse_sfc(source, file_name) Result~SfcDescriptor, SfcError~
-compile_template(file_name, template) Result~String, SfcError~
-compile_script(file_name, script) Result~String, SfcError~
-compile_styles(styles) Vec~StyleBlock~
}
SfcCompiler --> SfcModule : produces
SfcCompiler --> SfcError : handles
SfcModule --> StyleBlock : contains
```

**图表来源**
- [crates/iris-sfc/src/lib.rs:36-131](file://crates/iris-sfc/src/lib.rs#L36-L131)
- [crates/iris-sfc/src/lib.rs:161-240](file://crates/iris-sfc/src/lib.rs#L161-L240)

### 应用程序入口

应用程序入口负责协调整个热重载流程，从文件监控到 SFC 编译再到 GPU 资源更新。

```mermaid
sequenceDiagram
participant FS as 文件系统
participant FW as 文件监控器
participant HMR as HMR管理器
participant APP as 应用程序
participant SFC as SFC编译器
participant GPU as GPU渲染器
FS->>FW : 文件变更事件
FW-->>HMR : FileChange 事件
HMR->>HMR : 分类处理变更
HMR->>SFC : 编译受影响模块
SFC-->>HMR : 编译结果
HMR->>GPU : 更新 GPU 资源
GPU-->>HMR : 渲染完成
HMR-->>APP : 推送HMR事件
APP-->>FS : 继续监听
Note over APP,SFC : 编译错误时进行回滚
```

**图表来源**
- [crates/iris-app/src/main.rs:237-403](file://crates/iris-app/src/main.rs#L237-L403)

**章节来源**
- [crates/iris-gpu/src/file_watcher.rs:42-100](file://crates/iris-gpu/src/file_watcher.rs#L42-L100)
- [crates/iris-sfc/src/lib.rs:161-240](file://crates/iris-sfc/src/lib.rs#L161-L240)
- [crates/iris-app/src/main.rs:237-403](file://crates/iris-app/src/main.rs#L237-L403)

## 架构概览

Iris 文件监控与热重载系统采用分层架构设计，确保各个组件职责清晰、耦合度低。

```mermaid
graph TB
subgraph "表现层"
UI[用户界面]
Renderer[GPU渲染器]
HMRClient[浏览器HMR客户端]
end
subgraph "业务逻辑层"
HotReload[热重载控制器]
SFCCompiler[SFC编译器]
CacheManager[缓存管理器]
HMRManager[HMR管理器]
WebSocketManager[WebSocket管理器]
end
subgraph "数据访问层"
FileWatcher[文件监控器]
FileSystem[文件系统]
HTTPServer[HTTP服务器]
end
subgraph "基础设施层"
TokioRuntime[Tokio异步运行时]
WGPU[WGPU渲染引擎]
Notify[文件监控服务]
Axum[Axum HTTP引擎]
Broadcast[Broadcast通道]
End
UI --> Renderer
Renderer --> HotReload
HotReload --> SFCCompiler
HotReload --> CacheManager
HotReload --> HMRManager
HMRManager --> WebSocketManager
FileWatcher --> FileSystem
FileWatcher --> Notify
SFCCompiler --> TokioRuntime
Renderer --> WGPU
HotReload --> TokioRuntime
CacheManager --> TokioRuntime
WebSocketManager --> Broadcast
HTTPServer --> Axum
```

**图表来源**
- [crates/iris-app/src/main.rs:124-235](file://crates/iris-app/src/main.rs#L124-L235)
- [crates/iris-gpu/src/file_watcher.rs:245-411](file://crates/iris-gpu/src/file_watcher.rs#L245-L411)

系统的关键设计原则：

1. **异步非阻塞**：所有文件监控和编译操作都在异步环境中进行
2. **事件驱动**：基于事件的架构，文件变更触发相应的处理流程
3. **错误隔离**：编译错误不会影响主应用的运行，支持回滚机制
4. **资源管理**：智能的缓存管理和资源清理机制
5. **跨平台兼容**：统一的接口抽象，支持多操作系统
6. **模块级HMR**：支持样式的精确热替换和模块的热替换

## 详细组件分析

### 文件监控器实现

文件监控器实现了复杂的文件系统监听逻辑，包括防抖机制、事件去重和错误处理。

#### 防抖机制设计

```mermaid
flowchart TD
Start([开始监听]) --> Wait[等待文件事件]
Wait --> Receive[接收文件事件]
Receive --> UpdateDebounce[更新防抖状态]
UpdateDebounce --> CheckDelay{检查延迟时间}
CheckDelay --> |未到延迟| Wait
CheckDelay --> |已到延迟| FireEvent[触发事件]
FireEvent --> ReturnEvent[返回事件]
ReturnEvent --> Wait
subgraph "防抖状态"
LastEvent[上次事件时间]
Delay[延迟时间]
end
```

**图表来源**
- [crates/iris-gpu/src/file_watcher.rs:139-170](file://crates/iris-gpu/src/file_watcher.rs#L139-L170)

#### 事件处理流程

文件监控器支持多种文件事件类型，每种事件都有对应的处理逻辑：

```mermaid
flowchart TD
Event[文件事件] --> Type{事件类型}
Type --> |创建| Create[FileChange::Created]
Type --> |修改| Modify[FileChange::Modified]
Type --> |删除| Remove[FileChange::Removed]
Type --> |重命名| Rename[FileChange::Renamed]
Create --> Filter[扩展名过滤]
Modify --> Filter
Remove --> Filter
Rename --> Filter
Filter --> Channel[发送到事件通道]
Channel --> Debounce[防抖处理]
Debounce --> Return[返回事件]
```

**图表来源**
- [crates/iris-gpu/src/file_watcher.rs:318-367](file://crates/iris-gpu/src/file_watcher.rs#L318-L367)

#### 配置系统

文件监控器提供了灵活的配置选项：

| 配置项 | 默认值 | 描述 |
|--------|--------|------|
| watch_path | 当前目录 | 监听的根目录路径 |
| recursive | true | 是否递归监听子目录 |
| extensions | None | 文件扩展名过滤器 |
| channel_capacity | 2000 | 事件通道容量 |
| debounce_delay | 500ms | 防抖延迟时间 |

**章节来源**
- [crates/iris-gpu/src/file_watcher.rs:86-137](file://crates/iris-gpu/src/file_watcher.rs#L86-L137)
- [crates/iris-gpu/src/file_watcher.rs:413-463](file://crates/iris-gpu/src/file_watcher.rs#L413-L463)

### HMR 管理器实现

**更新** 新增模块级热重载管理器，负责文件监听和HMR事件处理。

#### HMR事件类型

```mermaid
classDiagram
class HmrEvent {
<<enumeration>>
Connected : message
FileChanged : path, file_name, timestamp
RebuildComplete : cleared_modules, duration_ms
ModuleUpdate : path, module_type, timestamp
StyleUpdate : path, css, timestamp
CompileError : message
NpmDownload : package, version, progress, status, error
}
class ModuleType {
<<enumeration>>
Vue
Script
Style
}
class PatchType {
<<enumeration>>
VueReload
CSSUpdate
FullReload
}
HmrEvent --> ModuleType : contains
HmrEvent --> PatchType : contains
```

**图表来源**
- [crates/iris-jetcrab-cli/src/server/hmr.rs:24-91](file://crates/iris-jetcrab-cli/src/server/hmr.rs#L24-L91)

#### 文件变更分类处理

HMR管理器根据文件类型采用不同的处理策略：

```mermaid
flowchart TD
FileChange[文件变更] --> Classify{文件类型分类}
Classify --> |入口文件| Entry[全量重载]
Classify --> |CSS/SCSS| Style[样式热替换]
Classify --> |Vue/TS/JS| Module[模块热替换]
Entry --> Broadcast[广播重建完成]
Style --> Compile[编译CSS]
Compile --> PushStyle[推送样式更新]
PushStyle --> Broadcast
Module --> Invalidate[使缓存失效]
Invalidate --> PushModule[推送模块更新]
PushModule --> Broadcast
Broadcast --> WebSocket[WebSocket推送]
```

**图表来源**
- [crates/iris-jetcrab-cli/src/server/hmr.rs:199-335](file://crates/iris-jetcrab-cli/src/server/hmr.rs#L199-L335)

#### 配置系统

HMR管理器提供了灵活的配置选项：

| 配置项 | 默认值 | 描述 |
|--------|--------|------|
| project_root | 项目根目录 | 监听的项目根路径 |
| enabled | true | 是否启用HMR功能 |
| debounce_delay | 300ms | 防抖延迟时间 |
| src_dir | src/ | 源码目录路径 |
| watch_extensions | vue,ts,tsx,js,jsx,mjs,css,scss,sass,less | 监听的文件扩展名 |

**章节来源**
- [crates/iris-jetcrab-cli/src/server/hmr.rs:129-175](file://crates/iris-jetcrab-cli/src/server/hmr.rs#L129-L175)
- [crates/iris-jetcrab-cli/src/server/hmr.rs:338-390](file://crates/iris-jetcrab-cli/src/server/hmr.rs#L338-L390)

### SFC 编译器实现

SFC 编译器负责将 Vue 单文件组件转换为可执行的 JavaScript 代码。

#### 模板编译流程

```mermaid
flowchart TD
Input[.vue 源码] --> Parse[解析 SFC 结构]
Parse --> ExtractTemplate[提取模板内容]
Parse --> ExtractScript[提取脚本内容]
Parse --> ExtractStyles[提取样式内容]
ExtractTemplate --> TemplateParser[HTML5Ever 解析]
TemplateParser --> VDOM[VNode AST]
VDOM --> TemplateCompiler[模板编译器]
TemplateCompiler --> RenderFn[渲染函数]
ExtractScript --> ScriptTranspiler[脚本转译器]
ScriptTranspiler --> JSCode[JavaScript 代码]
ExtractStyles --> StyleCompiler[样式编译器]
StyleCompiler --> CSSCode[CSS 代码]
RenderFn --> Module[SfcModule]
JSCode --> Module
CSSCode --> Module
```

**图表来源**
- [crates/iris-sfc/src/lib.rs:254-319](file://crates/iris-sfc/src/lib.rs#L254-L319)

#### 模板指令支持

**更新** 新增 v-text、v-html、v-show 三个 Vue 指令支持

SFC 编译器现已支持以下 Vue 指令：

| 指令 | 功能 | 示例 | 代码生成 |
|------|------|------|----------|
| v-if | 条件渲染 | `<div v-if="visible">内容</div>` | `condition ? element : null` |
| v-for | 列表渲染 | `<li v-for="item in items">{{ item }}</li>` | `source.map((iterator) => element)` |
| v-bind | 属性绑定 | `<input :value="message">` | `attrs[prop] = value` |
| v-on | 事件监听 | `<button @click="handleClick">点击</button>` | `attrs["onClick"] = handler` |
| v-model | 双向绑定 | `<input v-model="message">` | `value = variable, onInput = handler` |
| v-slot | 插槽 | `<slot v-slot:default="slotProps">内容</slot>` | `slot(name, props => element)` |
| v-once | 一次性渲染 | `<div v-once>静态内容</div>` | `_once: true` |
| v-pre | 跳过编译 | `<div v-pre>{{ 未编译的内容 }}</div>` | `text(rawHTML)` |
| v-cloak | 隐藏未编译内容 | `<div v-cloak>编译后显示</div>` | `attrs["v-cloak"] = ""` |
| v-memo | 记忆化优化 | `<div v-memo="[dep1, dep2]">内容</div>` | `_memo: "[dep1, dep2]"` |
| **v-text** | 文本内容设置 | `<span v-text="message"></span>` | `el.textContent = expression` |
| **v-html** | HTML 内容设置 | `<div v-html="rawHtml"></div>` | `el.innerHTML = expression` |
| **v-show** | 显示/隐藏切换 | `<div v-show="isVisible">内容</div>` | `el.style.display = condition ? '' : 'none'` |

**新增指令实现细节**：

1. **v-text 指令**：
   - 将元素的 `textContent` 属性设置为表达式的值
   - 适用于纯文本内容的动态更新
   - 与 `{{ }}` 插值类似但更灵活

2. **v-html 指令**：
   - 将元素的 `innerHTML` 属性设置为表达式的值
   - 支持动态 HTML 内容渲染
   - 需要注意 XSS 安全风险

3. **v-show 指令**：
   - 切换元素的 `display` 样式属性
   - 通过 CSS 控制显示/隐藏
   - 不会销毁 DOM 元素，性能更好

**章节来源**
- [crates/iris-sfc/src/template_compiler.rs:30-63](file://crates/iris-sfc/src/template_compiler.rs#L30-L63)
- [crates/iris-sfc/src/template_compiler.rs:220-233](file://crates/iris-sfc/src/template_compiler.rs#L220-L233)
- [crates/iris-sfc/src/template_compiler.rs:485-516](file://crates/iris-sfc/src/template_compiler.rs#L485-L516)
- [crates/iris-sfc/src/template_compiler.rs:663-716](file://crates/iris-sfc/src/template_compiler.rs#L663-L716)

### GPU 批渲染系统

GPU 批渲染系统优化了 2D 图形的渲染性能，通过合并多次绘制调用减少 GPU 调用开销。

#### 批渲染工作原理

```mermaid
flowchart TD
Submit[提交绘制命令] --> Pool[顶点池累积]
Pool --> Capacity{检查容量}
Capacity --> |容量不足| Panic[抛出异常]
Capacity --> |容量充足| AddVertex[添加顶点数据]
AddVertex --> AddIndex[添加索引数据]
AddIndex --> Flush{检查刷新条件}
Flush --> |需要刷新| Upload[上传到 GPU]
Upload --> Draw[执行绘制调用]
Draw --> Clear[清空缓冲区]
Clear --> Pool
Flush --> |不需要刷新| Pool
subgraph "GPU 缓冲区"
VertexBuffer[顶点缓冲区]
IndexBuffer[索引缓冲区]
end
AddVertex --> VertexBuffer
AddIndex --> IndexBuffer
```

**图表来源**
- [crates/iris-gpu/src/batch_renderer.rs:209-249](file://crates/iris-gpu/src/batch_renderer.rs#L209-L249)

#### 着色器实现

批渲染系统使用 WGSL 着色器实现高效的 2D 图形渲染：

```mermaid
graph LR
VS[顶点着色器] --> NDC[归一化设备坐标]
NDC --> Color[颜色插值]
Color --> FS[片段着色器]
FS --> Output[最终颜色]
subgraph "顶点数据"
Position[位置坐标]
ColorAttr[颜色属性]
UV[纹理坐标]
end
Position --> VS
ColorAttr --> VS
UV --> VS
```

**图表来源**
- [crates/iris-gpu/src/batch_shader.wgsl:9-25](file://crates/iris-gpu/src/batch_shader.wgsl#L9-L25)

**章节来源**
- [crates/iris-gpu/src/batch_renderer.rs:87-202](file://crates/iris-gpu/src/batch_renderer.rs#L87-L202)
- [crates/iris-gpu/src/batch_shader.wgsl:1-26](file://crates/iris-gpu/src/batch_shader.wgsl#L1-L26)

## 依赖关系分析

Iris 文件监控与热重载系统的依赖关系体现了清晰的分层架构：

```mermaid
graph TB
subgraph "应用层"
IrisApp[iris-app]
JetCrabCLI[iris-jetcrab-cli]
end
subgraph "业务逻辑层"
IrisGPU[iris-gpu]
IrisSFC[iris-sfc]
JetCrabEngine[iris-jetcrab-engine]
end
subgraph "核心层"
IrisCore[iris-core]
end
subgraph "外部库"
Notify[notify 7.0]
Tokio[tokio 1.0]
Axum[axum 0.7]
WS[websocket]
WGPU[wgpu 24.0]
Regex[regex 1.10]
RFD[rfd 0.15]
Broadcast[broadcast channel]
end
IrisApp --> IrisGPU
IrisApp --> IrisSFC
IrisApp --> IrisCore
JetCrabCLI --> JetCrabEngine
JetCrabCLI --> IrisCore
IrisGPU --> IrisCore
IrisSFC --> IrisCore
JetCrabEngine --> IrisCore
IrisGPU --> Notify
IrisGPU --> Tokio
IrisGPU --> WGPU
IrisGPU --> RFD
IrisSFC --> Regex
JetCrabCLI --> Axum
JetCrabCLI --> WS
JetCrabEngine --> Broadcast
subgraph "测试依赖"
UUID[uuid 1.0]
end
IrisGPU -.-> UUID
```

**图表来源**
- [Cargo.toml:13-29](file://Cargo.toml#L13-L29)
- [crates/iris-gpu/Cargo.toml:11-25](file://crates/iris-gpu/Cargo.toml#L11-L25)
- [crates/iris-sfc/Cargo.toml:11-23](file://crates/iris-sfc/Cargo.toml#L11-L23)

### 关键依赖说明

| 依赖库 | 版本 | 用途 | 重要性 |
|--------|------|------|--------|
| notify | 7.0 | 文件系统监控 | 核心依赖 |
| tokio | 1.0 | 异步运行时 | 核心依赖 |
| axum | 0.7 | HTTP服务器 | 核心依赖 |
| websocket | 0.24 | 实时通信 | 核心依赖 |
| wgpu | 24.0 | GPU 渲染 | 核心依赖 |
| regex | 1.10 | 正则表达式 | 重要依赖 |
| rfd | 0.15 | 图形对话框 | 可选依赖 |
| uuid | 1.0 | 测试工具 | 测试依赖 |
| broadcast | 内置 | 事件广播 | 核心依赖 |

**章节来源**
- [Cargo.toml:13-29](file://Cargo.toml#L13-L29)
- [crates/iris-gpu/Cargo.toml:11-25](file://crates/iris-gpu/Cargo.toml#L11-L25)

## 性能考虑

Iris 系统在设计时充分考虑了性能优化，采用了多种策略来确保高效的文件监控和热重载体验。

### 文件监控性能优化

1. **防抖机制**：默认 300ms 防抖延迟，避免编辑器保存时的重复触发
2. **事件去重**：同一文件的多次变更只保留最后一次
3. **通道容量管理**：默认 2000 个事件容量，可根据项目规模调整
4. **扩展名过滤**：只监听必要的文件类型，减少不必要的事件
5. **模块级HMR**：精确的文件变更分类，避免不必要的全量重载

### 编译性能优化

1. **正则表达式预编译**：使用 LazyLock 避免重复编译
2. **增量编译**：只编译发生变化的文件
3. **缓存机制**：SFC 模块缓存避免重复编译
4. **异步处理**：编译任务在后台线程执行
5. **编译缓存失效**：精确的模块缓存失效机制

### GPU 渲染性能优化

1. **批渲染**：合并多次绘制调用为单次 GPU 调用
2. **顶点缓冲区**：使用 GPU 本地缓冲区存储顶点数据
3. **索引缓冲区**：使用索引绘制减少顶点重复
4. **Alpha 混合**：硬件加速的颜色混合

### HMR性能优化

**更新** 模块级热重载系统提供了显著的性能提升：

1. **样式热替换**：CSS/SCSS 文件变更时直接替换样式，无需重新加载页面
2. **模块热替换**：Vue/TS 文件变更时精确替换模块，保持应用状态
3. **WebSocket通信**：高效的实时事件推送机制
4. **缓存失效**：精确的模块缓存失效，避免不必要的重新编译

## 故障排除指南

### 常见问题及解决方案

#### 文件监控器警告

**问题**：控制台出现 "File watcher event queue is full" 警告

**原因**：事件处理速度跟不上文件变更速度

**解决方案**：
1. 增加通道容量：`WatcherConfig::new(path).channel_capacity(5000)`
2. 优化事件处理逻辑
3. 减少不必要的文件监听

#### HMR连接问题

**问题**：浏览器无法连接到HMR WebSocket

**原因**：
1. HMR功能被禁用
2. 端口被占用
3. 网络连接问题

**解决方案**：
1. 检查配置文件中的 `hot_reload` 设置
2. 更改开发服务器端口
3. 检查防火墙设置

#### 样式热替换失败

**问题**：CSS/SCSS 文件变更后样式未更新

**解决方案**：
1. 确认文件扩展名为 `.css`、`.scss` 或 `.sass`
2. 检查样式编译是否成功
3. 验证浏览器是否正确接收HMR事件

#### 模块热替换失败

**问题**：Vue/TS 文件变更后模块未热替换

**解决方案**：
1. 确认文件扩展名正确（`.vue`、`.ts`、`.js`）
2. 检查模块导入路径
3. 验证浏览器是否支持模块热替换

**章节来源**
- [crates/iris-gpu/src/file_watcher.rs:290-391](file://crates/iris-gpu/src/file_watcher.rs#L290-L391)
- [crates/iris-app/src/main.rs:345-402](file://crates/iris-app/src/main.rs#L345-L402)

### 调试技巧

1. **启用详细日志**：设置环境变量 `RUST_LOG=debug`
2. **监控事件队列**：使用 `pending_events()` 方法检查队列状态
3. **测试文件监控**：参考 `test_hot_reload.md` 进行手动测试
4. **验证编译结果**：检查 SFC 编译输出的渲染函数
5. **检查HMR事件**：在浏览器开发者工具中查看WebSocket通信

**章节来源**
- [test_hot_reload.md:1-15](file://test_hot_reload.md#L1-L15)

## 结论

Iris 文件监控与热重载系统展现了现代 Rust 应用程序的最佳实践，通过精心设计的架构和优化策略，实现了高效、可靠的文件变更检测和响应机制。

### 主要成就

1. **完整的热重载链路**：从文件监控到 SFC 编译再到 GPU 资源更新的完整流程
2. **模块级HMR架构**：支持样式和模块的精确热替换，显著提升开发体验
3. **高性能设计**：异步非阻塞架构和多种性能优化策略
4. **跨平台兼容**：统一的接口抽象支持多操作系统
5. **错误处理机制**：完善的错误隔离和回滚机制
6. **可扩展性**：模块化设计便于功能扩展和维护

### 技术亮点

- **智能防抖**：默认 300ms 防抖延迟，避免编辑器保存时的重复触发
- **事件去重**：提高事件处理效率
- **异步架构**：充分利用现代硬件性能
- **缓存管理**：智能的资源管理和清理
- **跨平台支持**：统一的文件监控接口
- **模块级HMR**：支持样式的精确热替换和模块的热替换

**更新** 新增的模块级热重载系统是本次升级的核心成就，通过支持CSS/SCSS样式热替换和Vue/TypeScript模块热替换，为开发者提供了更加流畅和高效的开发体验。系统现在能够根据文件类型智能选择最合适的HMR策略，既保证了开发效率，又确保了应用的稳定性。

该系统为 Vue SFC 开发提供了卓越的开发体验，毫秒级的热重载响应和稳定的错误处理机制使其成为现代前端开发的理想选择。随着功能的不断完善，Iris 系统将继续为开发者提供更好的开发工具和更流畅的开发体验。