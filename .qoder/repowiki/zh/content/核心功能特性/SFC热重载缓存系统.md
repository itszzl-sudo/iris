...
## 更新摘要
**变更内容**
- 更新模板编译器章节，反映新增的v-text、v-html、v-show三个Vue指令支持
- 更新支持的Vue指令列表，包含最新的指令功能
- 新增指令处理详细说明和代码生成示例
...
**图表来源**
- [lib.rs:1-702](file://crates/iris-sfc/src/lib.rs#L1-L702)
...
**更新** 支持的Vue指令现已扩展至11个指令，包括新增的v-text、v-html、v-show指令：
...
**v-text指令**：将表达式的值设置为元素的textContent属性，适合纯文本内容显示
- 代码生成：`el.textContent = expression`
- 适用场景：避免闪烁渲染，直接设置纯文本内容

**v-html指令**：将表达式的值设置为元素的innerHTML属性，允许插入HTML标记
- 代码生成：`el.innerHTML = expression`
- 适用场景：动态HTML内容渲染，需注意XSS安全风险

**v-show指令**：通过切换元素的display样式属性实现显示/隐藏
- 代码生成：`el.style.display = condition ? '' : 'none'`
- 适用场景：频繁切换显示状态的元素，性能优于v-if

**章节来源**
- [template_compiler.rs:30-69](file://crates/iris-sfc/src/template_compiler.rs#L30-L69)
...
Template->>Template : 处理新增指令 (v-text/v-html/v-show)
...
CheckDirectives --> |v-text| VText[文本内容处理] ✨
CheckDirectives --> |v-html| VHtml[HTML内容处理] ✨
CheckDirectives --> |v-show| VShow[显示控制处理] ✨
...
**更新** 支持的指令处理现已扩展，新增三个指令的处理逻辑：

**新增指令处理详解：**

**v-text指令处理**：
- 代码生成：`(() => { const el = h("tag", attrs); el.textContent = expression; return el; })()`
- 特点：直接设置textContent，避免闪烁渲染
- 适用场景：纯文本内容显示，性能优于{{}}插值

**v-html指令处理**：
- 代码生成：`(() => { const el = h("tag", attrs); el.innerHTML = expression; return el; })()`
- 特点：直接设置innerHTML，支持HTML标记
- 安全考虑：需注意XSS攻击风险，建议配合内容安全策略

**v-show指令处理**：
- 代码生成：`(() => { const el = h("tag", attrs); el.style.display = condition ? '' : 'none'; return el; })()`
- 特点：通过切换display样式实现显示/隐藏
- 性能优势：频繁切换时比v-if更高效
...
**新增功能亮点**：
- **v-text指令**：提供高效的纯文本内容设置
- **v-html指令**：支持动态HTML内容渲染
- **v-show指令**：实现高性能的显示/隐藏控制
