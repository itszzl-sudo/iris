...
## 更新摘要
**变更内容**
- 新增 DependencyScanner 模块，提供自动依赖扫描和问题检测功能
- 新增 HTTP API 接口，支持依赖问题扫描和自动修复
- 集成 NpmDownloader，实现自动依赖下载和版本管理
- 增强依赖管理的完整性和自动化程度
...
**更新** 该功能现已显著增强，新增了自动依赖扫描、问题检测和自动修复能力，包括新的 DependencyScanner 模块和完整的 HTTP API 接口。
...
### 4. 项目扫描器 (ProjectScanner)

扫描和解析 Vue 项目的目录结构，识别项目配置和构建工具类型。

### 5. 编译器缓存 (CompilerCache)
...
Cache->>Cache : 检查缓存
alt 缓存未命中
Cache->>Scanner : 扫描依赖问题
Scanner->>Scanner : 检查未安装包
Scanner->>Downloader : 下载缺失的包
Downloader->>FS : 安装到node_modules
Scanner->>Engine : 构建依赖树
...
**图表来源**
- [routes.rs:554-680](file://crates/iris-jetcrab-cli/src/server/routes.rs#L554-L680)
...
**章节来源**
- [routes.rs:506-542](file://crates/iris-jetcrab-cli/src/server/routes.rs#L506-L542)
...
依赖树管理功能作为 Iris JetCrab 引擎的核心组件，经过显著增强后，成功实现了 Vue 项目依赖关系的智能化管理。通过精心设计的数据结构、高效的算法实现和完善的缓存机制，该功能为开发者提供了：
...
**更新** 本次更新显著提升了依赖管理的自动化程度和用户体验，使开发者能够更专注于代码编写，而无需担心复杂的依赖配置和管理问题。